  ...
  if (topicArrayLength.readInt8() > 1) {
    const topics = new Array(topicArrayLength.readInt8() - 1)
      ...
      .map((_) => {
        ...
        const recordBatch =
          logFileIndex === -1
            ? Buffer.alloc(4, 0) // Empty records length field
...
function readFromFileBuffer(topicName, partitionIndex, offset = 0, batchSize = 10) {
    try {
        ...
        if (!fs.existsSync(logFilePath)) {
            ...
            return Buffer.alloc(4, 0); // Return 4 bytes of zeros for records length
        }

        const logFile = fs.readFileSync(logFilePath);
        
        if (logFile.length === 0) {
            return Buffer.alloc(4, 0); // Return 4 bytes of zeros for empty records
        }
        
        // The log file already contains properly formatted record batches
        // We need to prefix it with the records length field (int32)
        ...
        return Buffer.alloc(4, 0); // Return 4 bytes of zeros on error
    }
}
